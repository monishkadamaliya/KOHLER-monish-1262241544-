...
class DesignOrchestrator:
    """Compose existing engines without moving factual decisions into the LLM."""
    ...
    def generate(self, request: DesignGenerateRequest) -> DesignGenerateResponse:
        ...
        if not categories:
            warnings.append("At least one product category is required for configuration generation.")
            ...
            result = self.retrieval.search(
                RetrievalRequest(
                    ...
                    space=SpacePreference(
                        max_width_mm=request.room_width_mm,
                        max_depth_mm=request.room_depth_mm,
                    ),
        ...
        solved = self.solver.search(search_request)
    ...
    def _design_reasons(objectives: dict[str, float], style: str | None, colour: str | None) -> list[str]:
        reasons = [
            "Passed deterministic catalogue, budget, dependency and placement validation.",
            f"Optimized across spatial, compatibility, budget, style, colour and functional objectives.",
        ]
        if style:
            ...
            "designs": [
                {
                    "rank": design.rank,
                    "skus": design.skus,
                    "placements": [placement.model_dump() for placement in design.placements],
                }
